...
### dirty 区域
```
dirty区域是：需要重绘的矩形区域（Rect），不是整个View，只是变化的部分
...
View.invalidateInternal(
    ...
    │
    ├── 计算dirty区域（View的bounds）
    ├── 标记 mPrivateFlags |= PFLAG_DIRTY
    │
    ▼
ViewGroup.invalidateChild(child, dirty)
    │
    ├── dirty区域坐标转换（子View→父View坐标系）
    ├── 与父View的dirty区域合并（union操作）
    └── 继续向上传递
    │
    ▼
...
performDraw()
    │
    ▼
canvas.clipRect(mDirty)  // 裁切到dirty区域
    │
    ▼
draw(canvas)
    │
    └── 只有和dirty区域相交的View才会重绘
        其他View的draw()被canvas裁切跳过
...
6.首次启动 activity 时不能在 onResume 中获取。measure 在 onResume 方法后执行（绘制流程就是在 resume 后开始执行的）。如果是从其他 activity 回到当前 activity 执行 onResume 方法就可以。
### MeasureSpec
...
此外，ViewGroup.onDraw 默认不执行，若要执行，需要 setWillNotDraw(false)、设置背景、设置前景、设置焦点高亮，其中一项满足即可。
# Touch
## onTouch
