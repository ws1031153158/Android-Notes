...
# JIT
Just-In-Time，即时编译，一种混合执行模式，它在程序运行时将热点字节码实时翻译成机器码并缓存，兼具了解释执行的启动速度和编译执行的高运行效率。    
先由解释器逐行执行，当代码被多次执行（即热点代码）时，JIT 编译器会将其编译为本地机器码以大幅提高后续执行速度。  
<img width="335" height="554" alt="image" src="https://github.com/user-attachments/assets/cdc0c947-8208-48da-9258-673c90d98931" />  
...
混合模式：  
虚拟机在程序启动时使用解释器逐条读取字节码并直接执行，无需生成独立的可执行文件。当某些方法或代码块频繁执行时，JIT 编译器会在运行时将其编译为与本地硬件相关的机器码。  
基于“二八定律”运行，即 20% 的代码占据了 80% 的执行时间。JIT 只对这 20% 的热点代码进行优化编译，平衡了编译时间和运行效率。  
需要预留内存。  

解释执行：  
不经过jit直接由解释器解释执行所有字节码，不加筛选的将全部代码进行编译机器码不论其执行频率是否有编译价值。  
执行效率不高。  

编译执行 (AOT)：  
预先编译成机器码，运行速度快但由于平台特性限制，失去了灵活性。  
AOT生成的二进制文件通常与特定的CPU架构和操作系统绑定。  
