...
这是 Handler 类的一个 实例方法  
默认处理路径：当消息到达时，如果没有设置 Callback，会直接调用该方法  
用法：通常通过继承 Handler 并重写 handleMessage 来处理消息  

2.Handler.Callback 的 handleMessage(Message msg)  
Handler.Callback 是一个 接口，可以在构造 Handler 时传入  
优先处理路径：当消息到达时，如果设置了 Callback，会先调用它的 handleMessage  
如果 Callback.handleMessage 返回 true，表示消息已被消费，不会再调用 Handler.handleMessage  
如果返回 false，则会继续调用 Handler.handleMessage  

```
public void dispatchMessage(@NonNull Message msg) {
    if (mCallback != null) {
        if (mCallback.handleMessage(msg)) {
            return; // Callback 消费消息，直接返回
        }
    }
    handleMessage(msg); // Callback 未消费消息，调用 Handler 的 handleMessage
}
```

3. 使用Callback  
轻量处理：不需要创建 Handler 子类，直接传入一个 Callback 实现即可  
多 Handler 共享逻辑：多个 Handler 可以复用同一个 Callback 实现  
...
Handler handler = new Handler(Looper.getMainLooper(), new Handler.Callback() {
    ...
    public boolean handleMessage(@NonNull Message msg) {
        ...
        return false; // 继续调用 Handler.handleMessage
    }
});
```
...
```
        ...
        public boolean handleMessage(Message msg) {
            ...
            try {
                ...
                reportError("EventIntercept_fail", new RuntimeException("ArbiterHook Event Intercept error", e));
            }

            // 多 callback 链式调用，根 callback 为 Handler 原始逻辑
            if (null != nextCallback) {
                return nextCallback.handleMessage(msg);
            }
            return false;
        }
    }
