...
## Config
配置管理模块，负责加载和管理路由配置，包括规则定义、动态更新等  
提供了一种基于文件的配置方式，使得开发者可以方便地管理和调整路由配置。为了确保配置的正确性和安全性，Config 模块还提供了一些校验机制，如配置格式校验、规则冲突检测等
### 规则定义
支持多种路由规则定义方式，如基于路径、标签、权重等。开发者可以根据实际需求选择合适的规则进行配置。规则定义采用了一种类似于 JSON 的配置格式，易于理解和维护。同时，为了提高可读性和可维护性，WMRouter 还支持使用注解方式定义路由规则
### 动态更新
当配置文件发生变更时，Config 模块会实时检测到并重新加载配置数据，确保路由规则的实时生效。
## Monitor
监控模块，用于收集路由过程中的各项指标数据，如请求量、响应时间等，以供分析和调优  
通过收集请求量、响应时间等数据，可以帮助开发者了解系统的运行状况，及时发现和解决潜在问题。Monitor 模块还提供了一种可视化界面，使得开发者可以直观地查看各项指标数据和图表
## Exporter
导出模块，将监控数据导出到第三方监控系统
# ARouter
## Jump
