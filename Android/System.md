...
# Window
Window 只是一个抽象概念，添加 window实际是添加 view  
```
 Window = 属性描述符 + View的管理者          │
│                                             │
│  它描述了：这块显示区域的规格（LayoutParams）│
│  它持有了：真正显示内容的 View（DecorView）  │
│  它沟通了：App 和 WMS 之间的契约            │
│                                             │
│  但它本身：不绘制、不显示、不是实体          │
│                                             │
│  真正的显示链路：                            │
│  View.draw() → Surface → SurfaceFlinger     │
│  Window 不在这条链路上 
```
## foundation
...
WindowType:  
```
Window Type 数值范围定义（WindowManager.LayoutParams）

┌─────────────────────────────────────────────────────┐
│  应用Window（Application Windows）                   │
│  TYPE_BASE_APPLICATION = 1                          │
│  TYPE_APPLICATION      = 2  ← 普通Activity          │
│  TYPE_APPLICATION_STARTING = 3  ← 启动窗口          │
│  范围：1 ~ 99                                        │
├─────────────────────────────────────────────────────┤
│  子Window（Sub Windows）                             │
│  TYPE_APPLICATION_PANEL = 1000  ← PopupWindow       │
...
│  ⚠️ 必须依附父Window存在                             │
├─────────────────────────────────────────────────────┤
│  系统Window（System Windows）                        │
│  TYPE_STATUS_BAR        = 2000  ← 状态栏             │
│  TYPE_SEARCH_BAR        = 2001                      │
│  TYPE_PHONE             = 2002  ← 悬浮窗(旧)         │
│  TYPE_TOAST             = 2005                      │
│  TYPE_SYSTEM_ALERT      = 2003  ← 需要权限           │
│  TYPE_SYSTEM_OVERLAY    = 2006                      │
│  TYPE_APPLICATION_OVERLAY = 2038 ← Android8+推荐    │
...
Z-Order:  
```
SurfaceFlinger 层级示意图（从下到上）

Layer 0:  Wallpaper（壁纸）
Layer 1:  Application Window（Activity）
Layer 2:  Application Window（另一个Activity）
Layer 3:  Sub Window（PopupWindow/Dialog）
Layer 4:  TYPE_APPLICATION_OVERLAY（悬浮窗）
Layer 5:  TYPE_STATUS_BAR（状态栏）
Layer 6:  TYPE_NAVIGATION_BAR（导航栏）
Layer 7:  TYPE_POINTER（鼠标指针）
```
...
```
    // 维护窗口栈，管理Z-Order
    private val windowStack = mutableListOf<PCWindow>()
    private var currentZOrder = 0
...
// 通过 WindowManager 添加自定义类型 Window
    fun addWindow(view: View, initialX: Int, initialY: Int, 
        ...
        val window = PCWindow(view, params, ++currentZOrder)
...
WindowManagerGlobal.addView()                     │
   │  ① 创建 ViewRootImpl                         │
   │  ② 将view/params/viewRootImpl存入列表         │
   │  ③ viewRootImpl.setView(view, params)        │
   ▼                                              │
...
ViewRootImpl.performTraversals()                  │
   │  measure → layout → draw                     │
   ▼                                              │
Surface.Canvas → SurfaceFlinger合成上屏
```

WindowManagerGlobal.addView:  
```
// WindowManagerGlobal 维护三个关键列表
private val mViews = ArrayList<View>()              // 所有根View
private val mRoots = ArrayList<ViewRootImpl>()      // 对应的ViewRootImpl
private val mParams = ArrayList<WindowManager.LayoutParams>() // 对应的参数

fun addView(view: View, params: WindowManager.LayoutParams, ...) {
    
    // 1. 参数合法性检查
    if (view.parent != null) {
        throw IllegalStateException("view已经有parent")
    }
    
    // 2. 子Window处理（如Dialog）
    // 如果是子Window类型，调整params，找到父Window的token
    adjustLayoutParamsForSubWindow(params)
    
    // 3. 创建ViewRootImpl
    val root = ViewRootImpl(view.context, display)
...
2.instrumentation 执行  execStartActivity，ATMS.getServive()获取ATMS实例  
3.ATMS 执行 startActivity（此时 AIDL 通信到 S 端了），调用到 resumeTopActivity，如果顶层 activity 为 null，则通过回调 Binder 通信到 ActivityThread 执行 transaction，其中的 handler （内部维护一个对象 H） 执行 sendMessage，由 AT 来 handleMessage，并执行 performLaunchActivity，在这一步获取 activity 信息，需要的话会使用 classLoader 来创建 activity，若第一次启动，则创建 application  
4.如果是新进程，则looper.prepareMainLooper 开始消息循环  
5.ApplicationThread执行scheduleLaunchActivity，后续由instrumentation控制Activity从onCreate到onResume  
