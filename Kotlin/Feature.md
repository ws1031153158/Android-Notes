...
是可挂起、可恢复 的计算单元，本质是对状态机 + 回调（Callback）的语法糖封装，运行在线程上，但不绑定特定线程。  
## 实现
...
7.LiveData 是 Android专属，天生感知生命周期，没有初始值也可以，主线程更新；而StateFlow不感知生命周期（需要配合 repeatOnLifecycle） ，必须有初始值  
8.LiveData是广播，是纯粹的数据容器；Flow是一对一，每次都完整执行（collect内部串行，多个collect互不影响）
### 背压
...
flow {
    repeat(100) { i ->
        emit(i)           // 发射
        // emit 是挂起函数！
        // 消费者没处理完（collect没处理完），emit 会挂起等待
        // 生产者自动被"限速"
    }
}.collect { value ->
    delay(100)            // 模拟慢消费
...
```
flow { } 块本身就是一个Lambda
没有collect之前，这个Lambda根本不执行

val flow = flow {
    println("开始执行")  // 没有collect，这行不会执行
    emit(1)
    emit(2)
}
...
// collect调用后，flow块才开始执行

原理：
flow { } 返回一个Flow对象
Flow对象只是持有了这个Lambda的引用
collect()调用时才真正执行Lambda
```
### flowOn
```
═══════════════════════════════════════════════════
...
Channel默认容量：
├── RENDEZVOUS（容量0）：emit必须等collect
├── BUFFERED（容量64）：可以提前生产
...
class HomeViewModel(
    ...
    val searchResults: StateFlow<List<Article>> = searchQuery
        ...
        .flowOn(Dispatchers.IO) //指定线程执行，collect则是在调用线程执行
        .stateIn(
...
### 生命周期
```
// Flow本身没有生命周期感知，需要配合lifecycleScope使用：
