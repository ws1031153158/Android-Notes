...
# 数组
不要只局限于基本数据类型的数据，数组类型也可以是个 List，但要注意初始化有些不同，如 List<Integer>[] l = new List[n]  
Arrays.sort 使用 lambda 时，注意时 () 不是 {}，此外传入的是数组本身，如 Arrays.sort(nums, (n1, n2) -> (n1 - n2))，如果是二维数组，则是 (n1[0] - n2[0]) 或者 (n1[1] - n2[1])，左 - 右 -> 为顺序  
需要求某种顺序或顺序关系时，考虑是不是可以针对下标操作，比如比较数值后将下标存起来，后续返回  
字母相关的集合，可以考虑直接创建一个大小为 26 的集合出来，可以计算每个字母的数量等参数，如 alp[char - 'A']++，此外，需要注意的是，char 转换为字母需要执行 char - 'A'/'a'
