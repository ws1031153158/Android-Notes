# 内存结构
Java 内存模型：  
分为主存和工作内存，所有变量都存在主存，线程有自己的工作内存，线程用主存中的变量时，会读一次主存，并拷贝到工作内存，运行时使用，线程执行完毕后，将最新的值更新到主存中。   
线程私有：  
虚拟机栈：线程每次调用方法会创建一个栈帧，结束时销毁，记录每个栈帧中的局部变量（表/数组），存放方法参数和方法内定义的局部变量，可通过索引直接访问指定成员、方法返回地址、常量池的相关引用（动态链接）以及操作数栈等，扩展时无法申请到足够的内存，就会抛出 OOM 异常。  
本地方法栈：调用操作系统原生方法（native）需要的内存区域，本地方法栈区域也会抛出 OOM 异常。  
程序计数器：寄存器，记录线程当前执行的指令信息，执行到哪条，下一条是哪个，所占空间的大小不会随程序的执行而发生改变，此内存区域不会发生内存溢出问题。  
...
## 引用类型
### 强引用
根路径直达，无论内存是否足够都不回收，JVM 停止才回收，用于维持对象一般状态，内存不足会直接抛异常，需要手动释放引用，如置为 null 或将其状态更改为 lifecycle 终止  
### 软引用
SoftReference，内存不足时回收，用于对象缓存，可以和构造时传入的引用队列关联，引用对象被回收将软引用加入引用队列，集中回收无效的软引用，可以用在 back 时缓存页面    
从网络上获取图片时，可以通过软引用缓存起来，但在实践中，使用软引用作为缓存时效率是比较低的，系统并不知道哪些软引用指向的对象应该被回收，哪些应该被保留，过早被回收的对象会导致不必要的工作，如 Bitmap 要重新从物理内存或者网络上加载到内存，更好的选择是使用LruCache
### 弱引用
WeakReference，不管当前内存空间足够与否，都会回收，用于对象缓存（移动端内存紧缺，一般采用弱引用），也可关联引用队列，用于很少使用的对象，如 ThreadLocal    
一般用来包装引用对象，避免 OOM 问题发生，此外还可使用 WeakHashMap，当 key 只有弱引用时，GC 发现后会自动清理键和值，作为简单的缓存表解决方案，例如在 ThreadLocal 中有使用，ThreadLocal.ThreadLocalMap.Entry  继承了弱引用，key 为当前线程实例，和 WeakHashMap 基本相同  
此外，内存泄漏监测框架 LeakCanary 也采用了弱引用与 ReferenceQueue 联合使用，ReferenceQueue 队列的具体作用就是当发生 GC 后，弱引用所持有的对象如果被回收就会进入该队列，所以只要在 activity onDestory 时，把 Activity 对象绑定在 WeakReference 中，然后手动执行一次 GC，然后观察 ReferenceQuery 中是不是包含对应的 Activity 对象，如果不包含，说明 Activity 被强引用，也就是发生了内存泄漏
### 虚引用
PhantomReference，get 方法直接返回 null，无法获取到被引用的对象，指向对象没有被其他强引用所持有，任何时候都可以回收，被回收后虚引用本身加入到引用队列（必须一起使用），记录对象已回收，用于跟踪对象的垃圾回收（能够收到系统通知为一个对象设置虚引用关联的唯一目的就是希望能在这个对象被收集器回收时收到一个系统通知），例如使用虚引用管理堆外内存  
### 引用队列
使用软/弱/虚引用时可以关联一个ReferenceQueue，当垃圾回收器准备回收一个被引用包装的对象时，该引用会被加入到关联的 ReferenceQueue，程序可以通过判断引用队列中是否已经加入引用，来了解被引用对象是否被 GC 回收  
### 堆外内存
NIO（同步非阻塞） 通过 byteBuffer.allocateDirect(size) 手动分配堆外内存、或反射获取 unSafe，通过 unSafe.allocateMemory 开辟堆外内存，byteBuffer 被虚引用对象 cleaner 引用，手动置为 null 后，虚引用加入引用队列，通过一个回收线程将队列中虚引用取出并调用 clean 方法，根据内存地址释放内存，不受 JVM GC 影响，需要手动回收。
## Algorithm
### 判断方式
#### 可达性分析
...
### 回收算法
#### 标记清除（mark-sweep）
标记待清理对象，在原地清除，速度快，流程简洁，会产生很多内存碎片，降低内存利用率
#### 标记拷贝（mark-copy）
内存对半分，将无需清除的对象复制到空的内存，另一半全部清空，避免了内存碎片问题，但内存浪费严重，内存利用率极低
#### 标记交换（mark-compact）
避免 sweep 和 copy 的缺点，清除垃圾后对剩余存活对象进行整理（移动），保证他们占用的内存空间连续，整理（移动）开销大，降低 GC 效率
#### 分代收集（generation-collection）
开始时对象分配在 eden 区（年轻代区分为eden区、to区、from区），eden 区放不下会发生 minor GC，标记不可达对象，将可达对象移动到 from区，在清理对象，经历多次 G C后，eden 和 from 区都可能有垃圾，此时将存活对象放在 to 区，然后清理垃圾，此时 to 区和 from 区会互换，对象每次移动区域年龄 +1，到达阈值后下次 minor GC 会转移到 tenured 区，老年代也满了会发成 Full GC，由于老年代对象通常较多，标记-清理-整理耗时长，会有卡顿现象，所以要尽量减少 Full GC。
## 内存碎片
